...
#include "config/rstoration_settings.h"
...
static const u32 sRSBrendan_Gfx[]         = INCBIN_U32("graphics/rstoration/intro/intro2_brendan.4bpp.lz");
static const u16 sRSBrendan_Pal[]         = INCBIN_U16("graphics/rstoration/intro/intro2_brendan.gbapal");
//static const u16 sRSBrendanBlue_Pal[]     = INCBIN_U16("graphics/rstoration/intro/intro2_brendan_blue.gbapal");
static const u32 sRSMay_Gfx[]             = INCBIN_U32("graphics/rstoration/intro/intro2_may.4bpp.lz");
static const u16 sRSMay_Pal[]             = INCBIN_U16("graphics/rstoration/intro/intro2_may.gbapal");
//static const u16 sRSMayBlue_Pal[]         = INCBIN_U16("graphics/rstoration/intro/intro2_may_blue.gbapal");
...
// TODO_RSTN check credits palettes to see if they're actually different
// TODO_RSTN otherwise can use the same define for intro AND credits

// 0 = Emerald Green, 1 = R/S Red, 2 = Mandela Blue
#if RSTN_INTRO_PLAYER_GFX == 0
    #define INTCRED_BRENDAN_GFX sBrendanCredits_Gfx
    #define INTCRED_MAY_GFX sMayCredits_Gfx
    #define INTCRED_BRENDAN_PAL sBrendanCredits_Pal
    #define INTCRED_MAY_PAL sMayCredits_Pal
#elif RSTN_INTRO_PLAYER_GFX == 1
    #define INTCRED_BRENDAN_GFX sRSBrendan_Gfx
    #define INTCRED_MAY_GFX sRSMay_Gfx
    #define INTCRED_BRENDAN_PAL sRSBrendan_Pal
    #define INTCRED_MAY_PAL sRSMay_Pal
#elif RSTN_INTRO_PLAYER_GFX == 2
    #define INTCRED_BRENDAN_GFX sRSBrendan_Gfx
    #define INTCRED_MAY_GFX sRSMay_Gfx
    #define INTCRED_BRENDAN_PAL sRSBrendanBlue_Pal
    #define INTCRED_MAY_PAL sRSMayBlue_Pal
#endif


// * 0 Emerald, no turn
// ! this needs adjustment so the frames dont switch, currently invis for duration
// gIntroBrendan_Gfx
// gIntroMay_Gfx
...
static const struct SpriteTemplate sSpriteTemplate_IntroLatios =
{
...
const struct CompressedSpriteSheet gSpriteSheet_RSTN_IntroBrendan[] =
{
    {
        .data = INTCRED_BRENDAN_GFX,
        .size = 0x3800,
        .tag = TAG_BRENDAN
    },
    {}
...
const struct CompressedSpriteSheet gSpriteSheet_RSTN_IntroMay[] =
{
    {
        .data = INTCRED_MAY_GFX,
        .size = 0x3800,
        .tag = TAG_MAY
    },
    {}
...
const struct CompressedSpriteSheet gSpriteSheet_RSTN_IntroBicycle[] =
{
    {
        .data = sBicycle_Gfx,
        .size = 0x1000,
...
{
    {
        .data = sLatios_Gfx, // ! gIntroFlygon_Gfx
        .size = 0x1000,
...
const struct CompressedSpriteSheet gSpriteSheet_RSTN_IntroFlygon[] =
{
    {
        // !!! config this
        .data = sLatias_Gfx, // ! gIntroFlygon_Gfx
        .size = 0x1000,
...
// !!! config palettes for eons
const struct SpritePalette gSpritePalettes_RSTN_IntroPlayerFlygon[] =
{
    { .data = INTCRED_BRENDAN_PAL, .tag = TAG_BRENDAN },
    { .data = INTCRED_MAY_PAL, .tag = TAG_MAY },
    { .data = sLatios_Pal, .tag = TAG_FLYGON_LATIOS },
    { .data = sLatias_Pal, .tag = TAG_FLYGON_LATIAS },
    {}
};

const struct CompressedSpriteSheet gSpriteSheet_RSTN_CreditsBrendan[] =
{
    {
...
const struct CompressedSpriteSheet gSpriteSheet_RSTN_CreditsMay[] =
{
    {
...
const struct CompressedSpriteSheet gSpriteSheet_RSTN_CreditsBicycle[] =
{
    {
        .data = sBicycle_Gfx,
        .size = 0x1000,
...
const struct SpritePalette gSpritePalettes_RSTN_Credits[] =
{
...
const struct CompressedSpriteSheet gSpriteSheet_RSTN_CreditsRivalBrendan[] =
{
    {
        .data = sBrendanCredits_Gfx,
        .size = 0x2000,
...
const struct CompressedSpriteSheet gSpriteSheet_RSTN_CreditsRivalMay[] =
{
    {
        .data = sMayCredits_Gfx,
        .size = 0x2000,
...
EWRAM_DATA u16 gIntroCredits_RSTN_MovingSceneryVBase = 0;
EWRAM_DATA s16 gIntroCredits_RSTN_MovingSceneryVOffset = 0;
EWRAM_DATA s16 gIntroCredits_RSTN_MovingSceneryState = 0;
...
void RSTN_LoadIntroPart2Graphics(u8 scenery)
{
    ...
    gIntroCredits_RSTN_MovingSceneryState = INTROCRED_SCENERY_NORMAL;
...
void RSTN_SetIntroPart2BgCnt(u8 scenery)
{
    ...
    case 1:
        ...
        SetGpuReg(REG_OFFSET_DISPCNT, DISPCNT_MODE_0
                                    ...
                                    | DISPCNT_OBJ_ON);
        break;
    case 2: // ! this doesn't exist in Ruby (maybe one of the others?)
        SetGpuReg(REG_OFFSET_BG3CNT, BGCNT_PRIORITY(3)
                                   | BGCNT_CHARBASE(0)
                                   | BGCNT_16COLOR
                                   | BGCNT_SCREENBASE(6)
...
void RSTN_LoadCreditsSceneGraphics(u8 scene)
{
    LZ77UnCompVram(sGrass_Gfx, (void *)(BG_CHAR_ADDR(1)));
    LZ77UnCompVram(sGrass_Tilemap, (void *)(BG_SCREEN_ADDR(15)));
    switch (scene)
...
void RSTN_SetCreditsSceneBgCnt(u8 scene)
{
    SetGpuReg(REG_OFFSET_BG3CNT, BGCNT_PRIORITY(3)
                               | BGCNT_CHARBASE(0)
                               | BGCNT_16COLOR
...
static void Task_BicycleBgAnimation(u8 taskId)
{
    ...
    if (bg1Speed != 0)
    {
        ...
        SetGpuReg(REG_OFFSET_BG1VOFS, gIntroCredits_RSTN_MovingSceneryVBase + gIntroCredits_RSTN_MovingSceneryVOffset);
    }
    ...
    if (bg2Speed != 0)
    {
        ...
        if (gTasks[taskId].tMode != 0)
            SetGpuReg(REG_OFFSET_BG2VOFS, gIntroCredits_RSTN_MovingSceneryVBase + gIntroCredits_RSTN_MovingSceneryVOffset);
        else
            SetGpuReg(REG_OFFSET_BG2VOFS, gIntroCredits_RSTN_MovingSceneryVBase);
    }
    ...
    if (bg3Speed != 0)
    {
        ...
        SetGpuReg(REG_OFFSET_BG3VOFS, gIntroCredits_RSTN_MovingSceneryVBase);
    }
}

void RSTN_CycleSceneryPalette(u8 mode)
{
    u16 x;
    u16 y;
    switch (mode)
...
{
    s32 x;
    s16 state = gIntroCredits_RSTN_MovingSceneryState;
    ...
    if (state != INTROCRED_SCENERY_FROZEN)
    {
        ...
        case INTROCRED_SCENERY_NORMAL:
            ...
            if (sprite->tHasVerticalMove)
                sprite->y2 = -(gIntroCredits_RSTN_MovingSceneryVBase + gIntroCredits_RSTN_MovingSceneryVOffset);
            else
                sprite->y2 = -gIntroCredits_RSTN_MovingSceneryVBase;
            break;
        }
    }
}
...
// TODO_RSTN config this
u8 RSTN_CreateIntroBrendanSprite(s16 x, s16 y)
{
    u8 playerSpriteId = CreateSprite(&sSpriteTemplate_Brendan, x, y, 0);
    u8 bicycleSpriteId = CreateSprite(&sSpriteTemplate_BrendanBicycle, x, y + 8, 1);
...
{
    u8 playerSpriteId = CreateSprite(&sSpriteTemplate_May, x, y, 0);
    u8 bicycleSpriteId = CreateSprite(&sSpriteTemplate_MayBicycle, x, y + 8, 1);
    gSprites[bicycleSpriteId].sPlayerSpriteId = playerSpriteId;
    return playerSpriteId;
} 
...
{
    u8 leftSpriteId = CreateSprite(&sSpriteTemplate_IntroLatios, x - 32, y, 2);
    u8 rightSpriteId = CreateSprite(&sSpriteTemplate_IntroLatios, x + 32, y, 2);
...
{
    u8 leftSpriteId = CreateSprite(&sSpriteTemplate_FlygonLatias, x - 32, y, 2);
    u8 rightSpriteId = CreateSprite(&sSpriteTemplate_FlygonLatias, x + 32, y, 2);
    gSprites[rightSpriteId].sLeftSpriteId = leftSpriteId;
    StartSpriteAnim(&gSprites[rightSpriteId], 1);
    gSprites[rightSpriteId].callback = &SpriteCB_FlygonRightHalf;
    return leftSpriteId;
